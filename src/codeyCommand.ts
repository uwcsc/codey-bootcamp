...
export type CodeyCommandArgumentValueType =
  | string
  | number
  | boolean
  | User
...
export type CodeyCommandArguments = {
  [key: string]: CodeyCommandArgumentValueType;
};
...
const defaultBackendErrorMessage =
  "Codey backend error - contact a mod for assistance";
...
export class CodeyCommand extends SapphireCommand {
  ...
  public configureSlashCommandBuilder(
    builder: SlashCommandBuilder
  ): SlashCommandBuilder {
    ...
    for (const subcommandName in this.details.subcommandDetails) {
      setCommandSubcommand(
        builder,
        this.details.subcommandDetails[subcommandName]
      );
    }
    return builder;
  }
  ...
  public async messageRun(
    ...
    if (!commandDetails) {
      ...
      for (const subcommandDetail of Object.values(
        this.details.subcommandDetails
      )) {
  ...
  public async chatInputRun(
    ...
    const commandDetails =
      this.details.subcommandDetails[subcommandName] ?? this.details;
    ...
    const args: CodeyCommandArguments = Object.assign(
      ...
      ...commandDetails.options
        ...
        .map((commandOptionName) => {
          const commandInteractionOption =
            interaction.options.get(commandOptionName);
    ...
    try {
      let successResponse = await commandDetails.executeCommand!(
        client,
        interaction,
        args
      );
