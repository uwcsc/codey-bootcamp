...
class BootcampSelectTrackCommand extends MentorCommand {
      ...
      args: [
        {
          key: "category",
          prompt:
            "What category are you the most familiar with? Check the waiting rooms for the current categories.",
          type: "string",
        },
      ],
      guildOnly: true,
  ...
  async onRun(
    ...
    if (!mentor?.channel) {
      return message.say(
        "Join a voice call such as  https://discord.gg/JmUC4WC9Jk to choose a track"
      );
    }
    ...
    if (trackCategory) {
      ...
      return message.say(
        "That category does not exist. Check the waiting rooms for the current categories."
      );
    }
  }
}
