...
class BootcampFeedbackMessageCommand extends AdminCommand {
  ...
  async onRun(
    ...
    if (feedback) {
      ...
      if (feedback == "--none")
        return message.reply("The feedback dm messages are disabled.");
      message.say(
        "The feedback direct message for mentees will now look like this:"
      );
      return message.reply(feedback);
    } else {
      return message.say(
        "The feedback message cannot be empty, type --none to disable it."
      );
    }
  }
}
