...
const getMemberEmbed = async (uwid: UwIdType): Promise<MessageEmbed> => {
  ...
  const members = (await (await fetch(MEMBER_API)).json())
    .members as memberStatus[];
  ...
  if (foundMember) {
    return new MessageEmbed()
      ...
      .setTitle(title)
      .setDescription(
        `You're a CSC member! Hooray! ${getEmojiByName("codeyLove")}`
      );
  }

  const NOT_MEMBER_DESCRIPTION = `You're not a CSC member! ${getEmojiByName(
    "codeySad"
  )}
