...
const getInfoEmbed = async (): Promise<MessageEmbed> => {
  ...
  const githubRepositoryReleases = await getRepositoryReleases(
    "uwcsc",
    "codeybot"
  );
  const infoEmbed = new MessageEmbed()
    ...
    .setFooter({
      text: `App version: ${githubRepositoryReleases[0].tag_name}`,
    });
  return infoEmbed;
};
...
export const infoCommandDetails: CodeyCommandDetails = {
  ...
  aliases: [],
  description:
    "Get Codey information - app version, repository link and issue templates.",
